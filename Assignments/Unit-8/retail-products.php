<?php
	...
	try {
		...
		//echo $result['product_name'] ; 						// Testing server connection
	}
	catch (PDOException $pdoError) {
		echo "Errors: " . $pdoError->getMessage();
	}
			 ...
			 <div class="productBlock">
				...
				<?php
					if($product['product_inStock'] <= 10){
						//Displaying information when stock is lower than 10 units
					?>
						<p class="productLowInventory"><?php echo $product['product_inStock']?> In Stock!</p>
					<?php
					}else {
						?>
						<p class="productInventory"><?php echo $product['product_inStock']?> In Stock!</p>
					<?php
					}				
